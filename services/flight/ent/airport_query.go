...
type AirportQuery struct {
	...
	withOrigin      *FlightQuery
	withDestination *FlightQuery
...
func (aq *AirportQuery) QueryOrigin() *FlightQuery {
...
func (aq *AirportQuery) QueryDestination() *FlightQuery {
	query := (&FlightClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *AirportQuery) Clone() *AirportQuery {
	...
	return &AirportQuery{
		...
		withOrigin:      aq.withOrigin.Clone(),
		withDestination: aq.withDestination.Clone(),
...
func (aq *AirportQuery) WithOrigin(opts ...func(*FlightQuery)) *AirportQuery {
	query := (&FlightClient{config: aq.config}).Query()
...
func (aq *AirportQuery) WithDestination(opts ...func(*FlightQuery)) *AirportQuery {
	query := (&FlightClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withDestination = query
	return aq
}
...
func (aq *AirportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Airport, error) {
	...
	if query := aq.withOrigin; query != nil {
		if err := aq.loadOrigin(ctx, query, nodes,
			func(n *Airport) { n.Edges.Origin = []*Flight{} },
			func(n *Airport, e *Flight) { n.Edges.Origin = append(n.Edges.Origin, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withDestination; query != nil {
		if err := aq.loadDestination(ctx, query, nodes,
			func(n *Airport) { n.Edges.Destination = []*Flight{} },
			func(n *Airport, e *Flight) { n.Edges.Destination = append(n.Edges.Destination, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AirportQuery) loadOrigin(ctx context.Context, query *FlightQuery, nodes []*Airport, init func(*Airport), assign func(*Airport, *Flight)) error {
...
func (aq *AirportQuery) loadDestination(ctx context.Context, query *FlightQuery, nodes []*Airport, init func(*Airport), assign func(*Airport, *Flight)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Airport)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
